...
export default function OnboardingScreen() {
  ...
  const { t } = useLanguage();
  const { isAuthenticated, isLoading: authLoading, signIn, signUp, signInWithApple } = useAuth();
  ...
  // If already authenticated, go straight to dashboard — no onboarding, no subscription prompt
  useEffect(() => {
    ...
    if (isAuthenticated && !hasRedirected.current) {
      ...
      // On native, login to RevenueCat and go to dashboard
      if (Platform.OS !== 'web') {
        const setupAndRedirect = async () => {
          try {
            ...
            console.warn('[Onboarding] RevenueCat login failed:', err);
          }
  ...
  useEffect(() => {
    if (showDevLogin || checkingAuth) return;
  ...
  }, [currentIndex, showDevLogin, checkingAuth]);
  ...
  const handleTryForFree = async () => {
    ...
    try {
      ...
      // Step 4: Check if already subscribed before prompting
      const alreadySubscribed = await checkSubscription();

      if (!alreadySubscribed) {
        // Not subscribed — present the subscription
        const purchased = await purchaseWeekly();
        if (!purchased) {
          console.log('[Onboarding] Purchase cancelled, proceeding with trial');
        }
      } else {
        console.log('[Onboarding] Already subscribed, skipping purchase');
      }

      // Navigate to dashboard — auth state change handles this via useEffect
      // But force it in case the useEffect already fired
      router.replace('/(tabs)/dashboard');
    } catch (err: unknown) {
